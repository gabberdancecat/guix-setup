...
(setup (:pkg meow)
  ...
  (meow-setup)
  (setq meow-use-cursor-position-hack t) ; will this cause issues?
  ;; free-keys is very useful!
  (define-key meow-insert-state-keymap (kbd "C-g")
              (lambda ()
                (interactive)
                (meow-insert-exit) ; normal mode
                (meow-cancel-selection))) ; cancel selection
  (define-key meow-insert-state-keymap (kbd "<f5>")
              (lambda () (interactive)
                (meow-insert-exit) ; normal mode
                (meow-cancel-selection) ; cancel selection
                (save-buffer)))
...
(setup (:pkg general :straight t)
  :config
  (general-create-definer ri/leader-key-def
    :prefix "C-c"))
...
;; should which-key integ use (add-hook 'lsp-mode-hook #'lsp-enable-which-key-integration) ?
(use-package lsp-mode
  ;; :commands (lsp lsp-deferred)
  :hook (lsp-mode . ri/lsp-mode-setup)
  :init
  (setq lsp-keymap-prefix "C-c l")
  (setq lsp-clients-clangd-executable (executable-find "clangd")) ; fixes clang lsp
  :config
  ;; Uncomment the below three lines to reduce flashiness:
  (setq lsp-eldoc-render-all nil) ; show lots of info in minibuffer... disable? too flashy?
  (setq lsp-enable-symbol-highlighting t) ; highlights all instances when hover over
  (setq lsp-symbol-highlighting-skip-current t) ; depends on prev var: skip current highlight
  ;; (setq lsp-signature-auto-activate nil) ; what does this do? signatures?
  (setq lsp-inlay-hint-enable t)

  (lsp-enable-which-key-integration t)
  (setq lsp-idle-delay 0.5) ; def 0.5, rec 0.6 ... why?
  (global-set-key (kbd "C-c l") lsp-command-map) ; bc otherwise Meow space doesn't work
  (message "lsp-mode loaded!"))

;; provides ui-sideline, ui-peek, and ui-doc
;;  (HOW to implement doc show?)
(use-package lsp-ui
  :hook (lsp-mode . lsp-ui-mode)
  :custom
  (lsp-ui-sideline-enable t)
  (lsp-ui-sideline-show-hover nil) ; rec nil, ugly; show hover messages in sideline
  (lsp-ui-doc-show-with-cursor nil) ; rec nil, shows doc when point hover over
  (lsp-ui-doc-position 'bottom)) ; depends on prev variable
...
(defun ri/c-compile-and-run (&optional use-extra)
  ...
  (let* ((src (file-name-nondirectory (buffer-file-name)))
         ...
         (extra (let ((valid-files "") ; fill this up
                      (files '("leak_detector_c.c")))
                  (dolist (p files)
                    (if (file-exists-p 
         ...
         (run-command 
          (if (and use-extra ri/c-compile-inputs)
              (concat "gcc " extra " " src " -lm -o " exe " && ./" exe " && " 
                      ri/c-compile-inputs)
            (concat "gcc " extra " " src " -lm -o " exe " && ./" exe))))
    (save-buffer)
...
(use-package yasnippet
  :hook ((eglot-server-initialized lsp-mode) . yas-minor-mode)
  :config
  (yas-reload-all))
...
(use-package flycheck
  :hook ((eglot-server-initialized lsp-mode) . flycheck-mode))
...
(use-package dirvish
  ...
  (dirvish-quick-access-entries ; It's a custom option, `setq' won't work
   '(("h" "~/"                          "Home")
     ...
     ("c" "~/Notes/School/classes"      "classes")
     ("C" "~/Code/"                     "Code")
